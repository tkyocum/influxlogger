/*****
 *  
 * This SmartApp will send sensor data to InfluxDB Cloud, where you can view sensor data, create alerts,
 * dashboards, and use the Flux language to perform powerful data manipulation that can be extended
 * across data sources.
 * 
 * A free-tier InfluxDB Cloud account is required. You can sign up for one using the below link.
 * 
...
definition(
    name: "InfluxDB Cloud Logger",
    namespace: "tkyocum",
    author: "Tim Yocum",
    description: "Log SmartThings device states to InfluxDB Cloud",
    category: "My Apps",
    iconUrl: "https://raw.githubusercontent.com/tkyocum/influxlogger/master/images/InfluxData.png",
    iconX2Url: "https://raw.githubusercontent.com/tkyocum/influxlogger/master/images/InfluxData@2x.png",
    iconX3Url: "https://raw.githubusercontent.com/tkyocum/influxlogger/master/images/InfluxData@3x.png")
...
preferences {
    ...
    section ("InfluxDB Cloud details:") {
        input "prefDatabaseURL", "text", title: "URL", defaultValue: "https://us-west-2-1.aws.cloud2.influxdata.com", required: true
        input "prefDatabaseToken", "text", title: "Token", defaultValue: "", required: true
        input "prefDatabaseBucket", "text", title: "Bucket", defaultValue: "st", required: true
        input "prefDatabaseOrg", "text", title: "Org", defaultValue: "", required: true
    }
...
def updated() {
    ...
    // Database config:
    state.databaseURL = settings.prefDatabaseURL
    ...
    state.path = "/api/v2/write?org=${state.databaseOrg}&bucket=${state.databaseBucket}&precision=s"
    state.headers = [:] 
...
def handleModeEvent(evt) {
    ...
    postToInfluxCloud(data)
}

/**
 *  handleEvent(evt)
 *
 *  Builds data to send to InfluxDB Cloud.
...
def handleEvent(evt) {
    ...
    // Build data string to send to InfluxDB Cloud:
 ...
 *  postToInfluxCloud()
 *
 *  Posts data to InfluxDB Cloud.
 *
 **/
def postToInfluxCloud(data) {
    logger("postToInfluxCloud(): Posting data to InfluxDB Cloud: Host: ${state.databaseURL}, Org: ${state.databaseOrg}, Bucket: ${state.databaseBucket}, Data: [${data}]","debug")

     def url = "${state.databaseURL}${state.path}" 
        try {
          httpPost(uri: url, body: data, headers: ["Authorization": "Token ${state.databaseToken}" ]) { response ->
          ...
          logger("postToInfluxCloud(): Error:${hre.getResponse()?.getData()}", "error")
      } catch (Exception e) {	
          logger("postToInfluxCloud(): Something went wrong when posting: ${e}","error")
      }
  }  
 ...
 *
 *  Handles response from post made in postToInfluxCloud().
 **/
def handleInfluxResponse(physicalgraph.device.HubResponse hubResponse) {
    if(hubResponse.status >= 400) {
		logger("postToInfluxCloud(): Something went wrong! Response from InfluxCloud: Headers: ${hubResponse.headers}, Body: ${hubResponse.body}","error")
    }
}
 ...
 *
 *  Further info: https://docs.influxdata.com/influxdb/v1.7/write_protocols/line_protocol_reference/
 **/
 ...
 *  groups from a SmartApp currently (unresolved since 2014)
 * 
 *  GroupIds can be obtained from the SmartThings IDE under 'My Locations', unless
 *  you enjoy the same rooms and same IDs as mine, in which case that's a little
 *  weird and creepy.
 *
...
private getGroupName(id) {
    ...
    else if (id == '1c960e89-9ba5-44d1-bde6-5b8a15918f39') {return 'Third Floor'}
    else if (id == '2eaa014f-7a33-4f92-bd7f-09473d68f531') {return 'Master Bedroom'}
    else if (id == '3cff5462-8e9e-48ca-94f7-b658b6923f3c') {return 'Kitchen'}
    else if (id == '549180eb-e176-4c36-9406-a0381e8c371e') {return 'Garage'}
    else if (id == '72fec3a7-0e95-4833-8649-d19bd8098e8d') {return 'Entry Doors'}
    else if (id == 'adabd23e-d560-4828-88d0-88755db3ba7a') {return 'Tim Office'}
    else if (id == 'b287a703-83ff-4dbf-a7e7-d8a1e3e7918e') {return 'First Floor'}
    else if (id == 'fe01507c-ba7e-4d4e-b359-60df16fd3224') {return 'Living Room'}
    else {return 'Unknown'}    
}
